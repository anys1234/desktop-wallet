...
const LedgerPhantom = require(_path.resolve(__dirname, './LedgerPhantom'))
...
function createWindow () {
  ...
  const iconpath = _path.resolve(__dirname, '/client/phantom-icon.svg')
  ...
  ipcMain.on('ledger', (event, arg) => {
    if (arg.action === 'detect') {
      ...
      if (!ledgercomm) {
        ...
        try {
          let phantom = new LedgerPhantom(ledgercomm)
          if (arg.action === 'signMessage') {
            phantom.signPersonalMessage_async(arg.path, Buffer.from(arg.data)
              .toString('hex'))
            ...
            phantom.signTransaction_async(arg.path, arg.data)
          ...
          const result = {
            connected: false,
            message: 'Cannot connect to Phantom application'
          }
          event.returnValue = result
        }
      }
