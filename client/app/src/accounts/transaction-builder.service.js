;(function () {
  ...
  function TransactionBuilderService ($timeout, $q, networkService, accountService, ledgerService, gettextCatalog, utilityService) {
    ...
    function createTransaction (deferred, config, fee, createTransactionFunc, setAdditionalTransactionPropsOnLedger) {
      ...
      if (phantom.crypto.getAddress(transaction.senderPublicKey, networkService.getNetwork().version) !== config.fromAddress) {
    ...
    function createSendTransaction (config) {
      return prepareTransaction(config, (deferred, account, fees) => {
        ...
        // If the Ledger is being used the passphrase is irrelevant, but phantom-js
        ...
        createTransaction(deferred,
                          config,
                          fees.send,
                          () => phantom.transaction.createTransaction(config.toAddress,
                                                                  config.amount,
                                                                  config.smartbridge,
                                                                  secret,
     ...
     * where amount is expected to be in phantomtoshi
     */
      ...
      return new Promise((resolve, reject) => {
        accountService.getFees(false).then(fees => {
          const invalidAddress = transactions.find(t => {
            return !phantom.crypto.validateAddress(t.address, network.version)
          })
          ...
          const processed = Promise.all(
            transactions.map(({ address, amount, smartbridge }, i) => {
              return new Promise((resolve, reject) => {
                ...
                const transaction = phantom.transaction.createTransaction(address, amount, smartbridge, secret, secondpassphrase, undefined, fees.send)
                ...
                if (ledger) {
                  ...
                  if (phantom.crypto.getAddress(transaction.senderPublicKey, network.version) !== fromAddress) {
    ...
    function createSecondPassphraseCreationTransaction (config) {
      return prepareTransaction(config, (deferred, account, fees) => {
        if (account.balance < fees.secondsignature) {
          deferred.reject(gettextCatalog.getString(
              'Not enough {{ currency }} on your account \'{{ address }}\' you need at least {{ amount }} to create a second passphrase!',
              {
                ...
                amount: phantomtoshiToPhantom(fees.secondsignature)
              }
          ))
          return
        }
        ...
        createTransaction(deferred,
                          ...
                          () => phantom.signature.createSignature(config.masterpassphrase, config.secondpassphrase, fees.secondsignature))
      })
    }
    ...
    function createDelegateCreationTransaction (config) {
      return prepareTransaction(config, (deferred, account, fees) => {
        if (account.balance < fees.delegate) {
          deferred.reject(gettextCatalog.getString(
            'Not enough {{ currency }} on your account \'{{ address }}\' you need at least {{ amount }} to register delegate!',
            {
              currency: networkService.getNetwork().token,
              address: config.fromAddress,
              amount: phantomtoshiToPhantom(fees.delegate)
            }
          ))
          return
        }
        ...
        createTransaction(deferred,
                          config,
                          fees.delegate,
                          () => phantom.delegate.createDelegate(config.masterpassphrase, config.username, config.secondpassphrase, fees.delegate))
      })
    }
    ...
    function createVoteTransaction (config) {
      return prepareTransaction(config, (deferred, account, fees) => {
        if (account.balance < fees.vote) {
          deferred.reject(gettextCatalog.getString(
            'Not enough {{ currency }} on your account \'{{ address }}\' you need at least {{ amount }} to vote!',
            {
              currency: networkService.getNetwork().token,
              address: config.fromAddress,
              amount: phantomtoshiToPhantom(fees.vote)
            }
          ))
          return
        }
        ...
        createTransaction(deferred,
                          config,
                          fees.vote,
                          () => phantom.vote.createVote(secret, config.publicKeys.split(','), config.secondpassphrase, fees.vote),
                          (transaction) => { transaction.recipientId = config.fromAddress })
      })
    }

    function phantomtoshiToPhantom (value) {
      return utilityService.phantomtoshiToPhantom(value) + ' ' + networkService.getNetwork().token
    }
