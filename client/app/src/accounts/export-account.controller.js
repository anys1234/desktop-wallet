;(function () {
  ...
  function ExportAccountController ($scope, $filter, $mdDialog, accountService, toastService, utilityService, gettextCatalog, gettext, account, theme, PHANTOM_LAUNCH_DATE) {
    $scope.vm = {}
    ...
    $scope.vm.minDate = PHANTOM_LAUNCH_DATE
    ...
    function prepareFile (account, transactions, isInComplete) {
      ...
      $scope.fileContent = 'Account:,' + account.address + eol +
                           'Balance:,' + utilityService.phantomtoshiToPhantom(account.balance) + eol +
