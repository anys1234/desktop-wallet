;(function () {
  ...
  angular.module('phantomclient.accounts')
    .service('accountService', ['$q', '$http', 'networkService', 'storageService', 'ledgerService', 'gettextCatalog', 'gettext', 'utilityService', 'PHANTOM_LAUNCH_DATE', AccountService])
   ...
   * @constructor
   */
  function AccountService ($q, $http, networkService, storageService, ledgerService, gettextCatalog, gettext, utilityService, PHANTOM_LAUNCH_DATE) {
    const self = this
    const phantom = require(require('path').resolve(__dirname, '../node_modules/phantomjscore'))
    ...
    function showTimestamp (timestamp) { // eslint-disable-line no-unused-vars
      const date = utilityService.phantomStampToDate(timestamp)
    ...
    function createAccount (passphrase) {
      return new Promise((resolve, reject) => {
        const publicKey = phantom.crypto.getKeys(passphrase).publicKey
        const address = phantom.crypto.getAddress(publicKey, networkService.getNetwork().version)
    ...
    function formatTransaction (transaction, recipientAddress) {
      ...
      transaction.date = utilityService.phantomStampToDate(transaction.timestamp)
      if (transaction.recipientId === recipientAddress) {
        transaction.total = transaction.amount
      // if (transaction.type == 0) {
      //   transaction.label = gettextCatalog.getString("Receive Phantom")
      // }
      }
      ...
      transaction.humanTotal = utilityService.phantomtoshiToPhantom(transaction.total) + ''
    ...
    function getRangedTransactions (address, startDate, endDate, onUpdate) {
      const startStamp = utilityService.dateToPhantomStamp(!startDate ? PHANTOM_LAUNCH_DATE : startDate)
      const endStamp = utilityService.dateToPhantomStamp(!endDate ? new Date(new Date().setHours(23, 59, 59, 59)) : endDate)
    ...
    function isValidAddress (address) {
      return phantom.crypto.validateAddress(address, networkService.getNetwork().version)
    }
    ...
    function verifyMessage (message, publicKey, signature) {
      ...
      const ecpair = phantom.ECPair.fromPublicKeyBuffer(Buffer.from(publicKey, 'hex'))
      const ecsignature = phantom.ECSignature.fromDER(Buffer.from(signature, 'hex'))
    ...
    function signMessage (message, passphrase) {
      ...
      let hash = crypto.createHash('sha256')
      hash = hash.update(Buffer.from(message, 'utf-8')).digest()
      const ecpair = phantom.crypto.getKeys(passphrase)
    ...
    function createVirtual (passphrase) {
      const deferred = $q.defer()
      const address = phantom.crypto.getAddress(phantom.crypto.getKeys(passphrase).publicKey, networkService.getNetwork().version)
            ...
            if (virtual[folder]) {
              if (arguments.length === 1) {
                if (value === null) {
                  ...
                  virtual[folder].amount = utilityService.phantomToPhantomtoshi(value)
                }
              } else {
                return virtual[folder].amount === null ? '' : utilityService.phantomtoshiToPhantom(virtual[folder].amount, true)
              }
            }
          }
        }
