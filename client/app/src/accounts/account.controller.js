;(function () {
  ...
  function AccountController (
    ...
    electron.ipcRenderer.on('uri', (event, uri) => {
      $timeout(() => {
        const qrcodeElement = document.querySelector('phantom-qrcode')
    ...
    if (!self.network.themeDphantom) self.network.themeDphantom = false
    ...
    generateDynamicPalette((name) => {
      ...
      generateDphantomTheme()
    })

    // set dark mode
    // if (self.network.themeDphantom) {self.currentTheme = 'dark'}
    ...
    self.saveFolder = function (account, folder) {
      accountService.setToFolder(account.address, folder, utilityService.phantomToPhantomtoshi(account.virtual.uservalue(folder)()))
    }
    ...
    function generateDphantomTheme (themeName) {
    ...
    function generateDynamicPalette (callback) {
      ...
      vibrant.from(url).getPalette((err, palette) => {
        ...
        Object.keys(materialPalette).forEach((color) => {
          ...
          const darkVibrantDiff = vibrant.Util.hexDiff(materialPalette[color]['900']['hex'], palette.DphantomVibrant.getHex())
          darkVibrantRatio[color] = darkVibrantDiff
        })

        const isPhantomJpg = _path.basename(url) === 'Phantom.jpg'
        let primaryColor = isPhantomJpg ? 'red' : sortObj(darkVibrantRatio)[0]
    ...
    function manageBackgrounds () {
      ...
      const initialDphantomMode = currentNetwork.themeDphantom
      ...
      function deleteImage (evt, image) {
        ...
        if (image === initialBackground) {
          selectBackground(backgrounds['images']['Phantom'])
        } else {
      ...
      function cancel () {
        ...
        currentNetwork.themeDphantom = initialDphantomMode
        currentTheme = initialThemeView
      }

      function toggleDphantom (status) {
        currentNetwork.themeDphantom = status
        setDphantomMode()
      }

      function setDphantomMode () {
        if (currentNetwork.themeDphantom) {
    ...
    function showExchangeRate () {
      return self.network.cmcTicker || self.network.token === 'PHANTOM'
    }
    ...
    function createSecondPassphrase (selectedAccount) {
      ...
      function warnAboutSecondPassphraseFee () {
        accountService.getFees(true).then((fees) => {
          const secondPhrasePhantomtoshiVal = fees['secondsignature']
          const secondPhrasePhantomVal = utilityService.phantomtoshiToPhantom(secondPhrasePhantomtoshiVal, true)
          const confirm = $mdDialog.confirm({
            title: gettextCatalog.getString('Second Passphrase fee ({{ currency }})', {currency: networkService.getNetwork().symbol}),
            secondPhrasePhantomVal: secondPhrasePhantomVal,
            textContent: gettextCatalog.getString('WARNING! Second passphrase creation costs {{ cost }} {{ currency }}',
                                                  {cost: secondPhrasePhantomVal, currency: networkService.getNetwork().token}),
    ...
    function showValidateTransaction (selectedAccount, transaction, cb) {
      ...
      $scope.validate = {
        ...
        humanAmount: utilityService.phantomtoshiToPhantom(transaction.amount),
        totalAmount: utilityService.phantomtoshiToPhantom(parseFloat(transaction.amount) + transaction.fee, true)
      }
