;(function () {
  ...
  function NetworkService($q, $http, $timeout, storageService, timeService, toastService) {
    ...
    const phantom = require(_path.resolve(__dirname, '../node_modules/phantomjscore'))
    phantom.networks.phantomTestnet = {
    ...
    const mainNetPhantomJsNetworkKey = 'phantomTestnet'
    const devNetPhantomJsNetworkKey = 'testnet'
    ...
    phantom.crypto.setNetworkVersion(network.version || 23)
    ...
    function switchNetwork(newnetwork, reload) {
      ...
      if (!n) {
        n = {
          TESTNET: createNetworkFromPhantomJs(mainNetPhantomJsNetworkKey, 0x37, 111, 'url(assets/images/images/Phantom.jpg)')
        }
        storageService.setGlobal('networks', n)
      }
      if (reload) {
    ...
    function createNetworkFromPhantomJs(phantomJsNetworkKey, version, slip44, background) {
      const phantomJsNetwork = phantom.networks[phantomJsNetworkKey]

      return {
        phantomJsKey: phantomJsNetworkKey,
        nethash: phantomJsNetwork.nethash,
        peerseed: phantomJsNetwork.activePeer.schema + '://' + phantomJsNetwork.activePeer.ip + ':' + phantomJsNetwork.activePeer.port,
        token: phantomJsNetwork.token,
        symbol: phantomJsNetwork.symbol,
        explorer: phantomJsNetwork.explorer,
        version: version,
        slip44: slip44,
        forcepeer: false,
    ...
    function tryGetPeersFromPhantomJs() {
      if (!network.phantomJsKey) {
        return
      }

      const phantomjsNetwork = phantom.networks[network.phantomJsKey]
      if (!phantomjsNetwork) {
        return
      }
    ...
    function findGoodPeer(peers, index, isStaticPeerList) {
      ...
      if (!isStaticPeerList && !isPeerListValid()) {
        ...
        peers = tryGetPeersFromPhantomJs()
