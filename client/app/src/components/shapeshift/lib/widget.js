  ...
  app.directive('coinSelector', ['ShapeShiftApiService', function (ShapeShiftApiService) {
    return {
      ...
      scope: {
        ...
        getMphantometData: '=getMphantometData',
        amount: '=amount',
  ...
  app.directive('coinTrader', ['$interval', 'ShapeShiftApiService', function ($interval, ShapeShiftApiService) {
    return {
      ...
      controller: function ($scope, $q) {
        ...
        $scope.getMphantometDataIn = function (coin) {
          if (coin === $scope.coinOut) return $scope.getMphantometData($scope.coinOut, $scope.coinIn)
          return $scope.getMphantometData(coin, $scope.coinOut)
        }
        $scope.getMphantometDataOut = function (coin) {
          if (coin === $scope.coinIn) return $scope.getMphantometData($scope.coinOut, $scope.coinIn)
          return $scope.getMphantometData($scope.coinIn, coin)
        }
        $scope.getMphantometData = function (coinIn, coinOut) {
          $scope.coinIn = coinIn
        ...
        ShapeShiftApiService.coins().then((coins) => {
          ...
          $scope.getMphantometData($scope.coinIn, $scope.coinOut)
        })
  ...
  app.service('ShapeShiftApiService', ($q) => {
    ...
    return {
      ...
      marketInfo: function (coinIn, coinOut) {
        const promise = $q.defer()
        SSA.GetMphantometInfo(coinIn, coinOut, (data) => {
