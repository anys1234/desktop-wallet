;(function () {
  ...
  function AddressbookController ($scope, $mdDialog, toastService, storageService, gettextCatalog, gettext, accountService, utilityService) {
    ...
    self.isAddress = function (address) {
      return require(require('path').resolve(__dirname, '../node_modules/phantomjscore')).crypto.validateAddress(address)
    }
    ...
    self.getStats = function (account, contact) {
      ...
      if (transactions) {
        ...
        if (incomeTx.length > 0) {
          ...
          stats.income.amount = utilityService.phantomtoshiToPhantom(incomeAmount, false, 2)
        }
        ...
        if (expendTx.length > 0) {
          ...
          stats.expend.amount = utilityService.phantomtoshiToPhantom(expendAmount, false, 2)
        }
      }
